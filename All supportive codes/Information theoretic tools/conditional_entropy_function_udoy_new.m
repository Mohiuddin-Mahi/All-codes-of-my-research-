function [con_ent1,con_ent2,con_ent3]=conditional_entropy_function_udoy_new(x,y,nsymbols,k,l,d)
...
for i=1:nsymbols
    for j=1:nsymbols
        if(new_pr_x1x(i,j)~=0 && new_pr_x(j)~=0)
            con_ent1=con_ent1-(new_pr_x1x(i,j)*log2(new_pr_x1x(i,j)/new_pr_x(j)));
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j=1:nsymbols
        for k=1:nsymbols
            if(new_pr_x1xy(i,j,k)~=0 && new_pr_xy(j,k)~=0)
                con_ent2=con_ent2-(new_pr_x1xy(i,j,k)*log2(new_pr_x1xy(i,j,k)/new_pr_xy(j,k)));
            end
        end
    end
end
...
for i=1:nsymbols
    for k=1:nsymbols
        if(new_pr_x1y(i,k)~=0 && new_pr_y(k)~=0)
            con_ent3=con_ent3-(new_pr_x1y(i,k)*log2(new_pr_x1y(i,k)/new_pr_y(k)));
        end
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
