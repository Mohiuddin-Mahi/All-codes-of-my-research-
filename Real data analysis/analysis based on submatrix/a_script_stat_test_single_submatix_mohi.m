...
for frame_ind=500
    ...
    %cell_y=mat2cell(data_y,[153 153 153 153 153 153],[221 221 221 221 221 221]);
    cell_y=mat2cell(data_y(1:end-2,1:end-2),[229 229 229 229],[331 331 331 331]);
    ...
    for indy=1:size(cell_y,1)
        for indx=1:size(cell_y,2)
            ...
            data1=adjacent_grids_delete_function(cell_y,indy,indx);            %% Y data
            data2=adjacent_grids_delete_function(cell_x,indy,indx);            %% X data

            all_pr=zeros(length(l1_array),length(l2_array));
            true_te=zeros(length(l1_array),length(l2_array));
                ...
                parfor lambda_ind2=1:length(l2_array)
                    ...
                    if lambda1>lambda2
                        ...
                        for trail1=1:length(data2)
                            ...
                            if trail1==1
                                tr_pr{1,1}= pr_x_x_y;
                                tr_te(1,1)=calc_te_new_code(pr_x_x_y,nsymbols);
                            else
            ...
            save(['data_stat_test_submatrix/data_',num2str(indy),'_',num2str(indx),'_submatix_4by4.mat'],'true_te',...
                'surrogate_te','true_prob','surrogate_prob','pass_or_fail','all_pr','firing_y','firing_x')
        end
    end
