...
from dit.multivariate import total_correlation as I, intrinsic_mutual_information as IMI
import sys
from multiprocessing import Pool, freeze_support

n_jobs =30
file_str=str(sys.argv[1])
num_noise=int(sys.argv[2])
num_birds=int(sys.argv[3])

def calc1(arg):

    symbol, j, k, bird1_ind,bird2_ind= arg
    mystr2=str(k);
    a=np.array(symbol["symbols_data"][0][j][bird1_ind])
    b=np.array(symbol["symbols_data"][0][j][bird2_ind])
...
def calc2(arg):
    ...
    symbol, j,k,bird1_ind,bird2_ind= arg
    mystr1=str(k);
    a=np.array(symbol["symbols_data"][0][j][bird2_ind])
    b=np.array(symbol["symbols_data"][0][j][bird1_ind])
    array=np.vstack([a, b]).T
    time_series_distribution = dist_from_timeseries(array, history_length=1)
    time_series_distribution.set_rv_names(['X','Y','Z','T'])                        #X=x_t;Y=y_t;Z=x_(t+1);T=y_(t+1)
    intrinsic_x_to_y= IMI(time_series_distribution, ['X', 'T'], 'Y')
    ...
    for file_ind in range(1,num_noise+1):
        for bird1_ind in range(0,num_birds):
            for bird2_ind in range(bird1_ind+1, num_birds):
                ...
                symbol=loadmat("/home/udoy/Mar23/"+file_str+"_"+my_str1)
                row_num=np.shape(symbol["symbols_data"])[0]
                col_num=np.shape(symbol["symbols_data"])[1]
                ...
                shared_1 = res[:, :, 2]
                synergistic_1 = res[:, :, 3]
                savemat("/home/udoy/Mar23/Results" + file_str+str(file_ind)+"_"+str(bird1_ind+1)+"_"+str(bird2_ind+1)+"_" +my_str1,{"intrinsic_w_2_"+my_str2:intrinsic,"synergistic_w_2_"+my_str2:synergistic,"Transfer_entropy_w_2_"+my_str2:TE,"shared_w_2_"+my_str2:shared,"TDMI_w_2_"+my_str2:TDMI,"r_intrinsic_w_2_"+my_str2:intrinsic_1,"r_synergistic_w_2_"+my_str2:synergistic_1,"r_Transfer_entropy_w_2_"+my_str2:TE_1,"r_shared_w_2_"+my_str2:shared_1,"r_TDMI_w_2_"+my_str2:TDMI_1})
