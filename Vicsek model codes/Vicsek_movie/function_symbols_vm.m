...
function function_symbols_vm(tmax,r,dt,vs,lbox,nbird,nsymbols,num_trail,num_noise,num_coupling,...
    coup_min,coup_max,noise_min,noise_max)

noise_array=linspace(noise_min,noise_max,num_noise);
...
for coup_ind=1:length(coup_array)
    ...
    for noise_ind=1:length(noise_array)
        ...
        parfor trail_ind=1:num_trail
            ...
            x_data{trail_ind}=x_out;
            y_data{trail_ind}=y_out;
            symbols_real{trail_ind}=theta_out;
            symbols{trail_ind}=get_symbol_strings(theta_out,nsymbols);
        end
       save(['symbols/symbols_r3_box10_coup_',num2str(coup_ind),'_noise_',num2str(noise_ind)],'symbols','symbols_real',...
           'x_data','y_data','-v7.3')
        toc;
    end
end
