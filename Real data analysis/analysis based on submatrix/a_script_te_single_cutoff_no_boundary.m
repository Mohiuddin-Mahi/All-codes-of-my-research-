...
%                     im_data{lambda_ind}(i,j)=1;
%                 elseif(data1(i,j)==0 && data2(i,j)==1)
%                     im_data{lambda_ind}(i,j)=2;
%                 elseif(data1(i,j)==1 && data2(i,j)==0)
%                     im_data{lambda_ind}(i,j)=3;
%                 else
%                     im_data{lambda_ind}(i,j)=4;
%                 end
% 
%             end
%         end
...
% 
% save(['data_te_single_cutoff/te_resize_0.2_no_boundary_frame_',num2str(frame_ind),'.mat'],'trans_ent')



% % %%%%%%%%%%%% Figure %%%%%%%%%%%%%%%%
% % 
% % load('data_te_single_cutoff/te_resize_0.2_no_boundary_frame_500.mat')
% % 
...
% % %%% Transfer Entropy %%
% % figure
% % h(1)=plot(5*lambda_data,trans_ent,'LineWidth',1.5);
% % xlim(5*[1 max(lambda_data)])
...
% % figure
% % h(2)=plot(5*lambda_data(2:end),derivative1_TE,'LineWidth',1.5);
% % xlim(5*[1 max(lambda_data)])
% % xlabel('Cutoff distance ($\lambda$)','interpreter','latex')
...
% % figure
% % h(3)=plot(5*lambda_data(3:end),derivative2_TE,'LineWidth',1.5);
% % xlim(5*[1 max(lambda_data)])
% % xlabel('Cutoff distance ($\lambda$)','interpreter','latex')
