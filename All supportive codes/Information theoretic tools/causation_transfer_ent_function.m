...
function [transfer_ent_z_x,causation_ent_z_x]=causation_transfer_ent_function(x,y,z,tau,nsymbols)
...
[tmp_x1xyz_1,~,tmp_x1xyz_3]=unique(vec_x1xyz,'rows');                             %%% [C,ia,ic]=unique(A,'rows') i.e., C is unique matrix of A, 
                                                                                  %%%% ia is index of C's rows in A, ic is index of A's rows in C.
[a,~]=size(tmp_x1xyz_1);
...
for i=1:nsymbols
    for j=1:nsymbols
        for l=1:nsymbols
            if(new_pr_x1xz(i,j,l)~=0 && new_pr_x1x(i,j)~=0 && new_pr_xz(j,l)~=0 && new_pr_x(j)~=0)
                transfer_ent_z_x=transfer_ent_z_x+new_pr_x1xz(i,j,l)*log2(new_pr_x1xz(i,j,l)*new_pr_x(j)/...
                    (new_pr_xz(j,l)*new_pr_x1x(i,j)));
            end
        end
    end
end
...
%%%%%%%%%%%%%%% Causation entropy computation %%%%%%%%%%%%%%%%%%%%%%%%%%
causation_ent_z_x=0;
for i=1:nsymbols
    for j=1:nsymbols
        for k=1:nsymbols
            for l=1:nsymbols
                if(new_pr_x1xyz(i,j,k,l)~=0 && new_pr_x1xy(i,j,k)~=0 && new_pr_xyz(j,k,l)~=0 && new_pr_xy(j,k)~=0)
                    causation_ent_z_x=causation_ent_z_x+new_pr_x1xyz(i,j,k,l)*log2(new_pr_x1xyz(i,j,k,l)*new_pr_xy(j,k)/...
                        (new_pr_xyz(j,k,l)*new_pr_x1xy(i,j,k)));
                end
            end
        end
    end
