...
function [new_pr_x_x_y,transfer_entropy_y_x]=transfer_entropy_function(x,y,nsymbols,tau)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
transfer_entropy_y_x=0;
for i=1:nsymbols
    for j=1:nsymbols
        for k=1:nsymbols
            if(new_pr_x_x_y(i,j,k)~=0 && new_pr_x_x(i,j)~=0 && ...
                    new_pr_x_y(j,k)~=0 && new_pr_x(j)~=0)
                transfer_entropy_y_x=transfer_entropy_y_x+new_pr_x_x_y(i,j,k)*log2(new_pr_x_x_y(i,j,k)*new_pr_x(j)/...
