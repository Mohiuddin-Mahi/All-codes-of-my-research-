...
pr_xy=zeros(length(symbols),length(symbols));
pr_zx=zeros(length(symbols),length(symbols));
pr_zy=zeros(length(symbols),length(symbols));
pr_zyx=zeros(length(symbols),length(symbols),length(symbols));
...
for ind3=1:length(symbols)             
    for ind2=1:length(symbols)
        for ind1=1:length(symbols)
            if  pr_y(ind2) ~=0 && pr_xy(ind1,ind2)~=0  && pr_zy(ind3,ind2)~=0 && pr_zyx(ind3,ind2,ind1)~=0
