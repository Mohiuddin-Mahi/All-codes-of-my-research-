...
for ind=2:TMAX
    if status(ind)<p_leader
        x(ind)=x(ind-1);            %% what will be the x(1)?? Is it 0 or 1?
    else
        x(ind)=randi(2)-1;
    end
    ...
    for f_ind=1:nbird-1
        ...
        if curr_status<p_leader
            ...
            followers(f_ind,ind)=followers(follower_num,ind-1);   %%% How the values of followers are taken??
        end
    end
end

symbols(:,1)=uint8(x);
symbols(:,2)=uint8(followers(1,:));             %%% why has the 1st row of followers been selected??
