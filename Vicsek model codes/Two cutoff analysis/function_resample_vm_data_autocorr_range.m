...
function function_resample_vm_data_autocorr_range(d,num_trail,noise_array,corr_threshold)
...
for noise_ind=1:4%length(noise_array)
    ...
    for trail_ind=1:num_trail
        ...
        [optimal_lags,~]=time_delay_counting_vm_autocorr_range(auto_corr{trail_ind},corr_threshold);

        tc=round(mean(optimal_lags));
    ...
    x_value=new_x_val;
    y_value=new_y_val;
    save(['symbols_resample_autocorr_range/symbols_noise_',num2str(noise_ind),'.mat'],'symbols','x_value',...
        'y_value','-v7.3')
end