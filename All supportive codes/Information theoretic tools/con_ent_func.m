...
function [con_ent_x,con_ent_y,p_xy] = con_ent_func(x,y,symbols)
p_xy=zeros(length(symbols),length(symbols));        %% For large data pc may be slow so this statement is used to make the pc faster
...
for ind1=1:length(symbols)
    for ind2=1:length(symbols)
        ...
        con_ent_x=con_ent_x-(p_xy(ind1,ind2).*log2(p_xy(ind1,ind2)/p_x(ind1)));
        con_ent_y=con_ent_y-(p_xy(ind1,ind2).*log2(p_xy(ind1,ind2)/p_y(ind2)));
        end
    end
end
end
